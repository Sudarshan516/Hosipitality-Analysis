...
CHANGE check_in_date `date` DATE;
ALTER TABLE fact_bookings 
CHANGE room_category room_id VARCHAR(20);
...
-- Average rating
select concat(round(avg(ratings_given),2),'%') as `Average Rating`from fact_bookings;
...
left JOIN fact_bookings
  ON dim_date.date = STR_TO_DATE(TRIM(fact_bookings.date), '%Y-%m-%d')
...
join
 ...
 select booking_platform,concat(round(sum(fact_bookings.revenue_realized)/1000000,2),'M') as `Total revenue` from fact_bookings
...
-- weekly trend (Revenue,Cancellation)
select dt.`week no`,concat(round(sum(fb.revenue_realized)/1000000,2),'M')as `Total Revenue`,
    concat( round(COUNT(CASE WHEN booking_status = 'Cancelled' THEN 1 END) * 100
    / COUNT(*),2),'%') AS `Cancellation`
    from dim_date dt
left  join 
 ...
 -- weekly trend (bookings,Occupancy)
select dt.`week no`,
concat(round((sum(fab.successful_bookings)/1000),2),'k')`Total Bookings`,
...
group by `week no`;
 ...
 -- weekly trend table( losting connection)
